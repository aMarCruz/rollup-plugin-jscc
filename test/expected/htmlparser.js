...
assign(HtmlParser.prototype, {
  ...
  */
  _pushText (data, pos, nonew) {
    ...
    if (q && q.type === 3) {
      ...
      state.queued = this._newNode(3, null, pos, pos + len)
    }
  },
  ...
  */
  _flush () {
    ...
    if (q) {
      if (q.type === 1 && this.options.verbose) {
  ...
  */
  _parseText (state) {
    ...
    if (state.scryle) {
      ...
      this._write(this._newNode(1, '/' + name, start, next))
      state.mode = 3
  ...
  */
  _parseTag (state) {
    ...
    if (str[0] === '!') {
      ...
      let tag = this._newNode(1, name, pos - 1, endPos)
  ...
  */
  _parseComment (state, pos) {
    ...
    if (pos < 0) {
      pos = this.errorHandler(state, `unclosed ${ type === 10 ? 'doctype' : 'comment' }`)
    }
    pos += str.length

    let node = this._newNode(type, null, loc, pos)

    if (type === 8 && !this.options.keepComments) {
  ...
  */
  ...
  _newNode (type, name, start, next) {
    ...
    if (this.options.verbose || ~[3, 8, 10].indexOf(type)) {
