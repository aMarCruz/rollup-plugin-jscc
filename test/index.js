...
const fixturePath = name => {
  const file = path.join(__dirname, 'fixtures', name).replace(/\\/g, '/')

  return path.extname(file) ? file : `${file}.js`
}

const generate = (file, opts) => {
...
const testFileStr = (file, expected, opts) => {
  opts = opts || {}
...
describe('rollup-plugin-jscc', function () {
  ...
  it('predefined `_FILE` value is the relative path of the current dir', function () {
    return rollupFile('def-file-var').then(({ code }) =>
      expect(code).to.contain('// fixtures/def-file-var.js\n')
    )
  })
...
describe('Options:', function () {
  it('default preprocess files with extensions [.js, .jsx, .ts, .tsx, .tag]', function () {
    ...
    return transformer()
      .transform('//', 'a.ts')
      .then(res => expect(res).to.be.an(Object).and.have.property('code', '//'))
  })

  it('`extensions:"*"` (string) must include all the files', function () {
    return transformer({ extensions: '*' })
      .transform('<a/>', 'a.html')
